...
TAHtml.createFlavor = function createFlavor() {
  var domain = "device.teslaandroid.com";
  var secure = true;

  try {
    var params = new URLSearchParams(global.location.search);
    var paramDomain = params.get("domain");
    var storedDomain = global.localStorage && global.localStorage.getItem("ta_domain");
    if (paramDomain) {
      domain = paramDomain;
      if (global.localStorage) global.localStorage.setItem("ta_domain", paramDomain);
    } else if (storedDomain) {
      domain = storedDomain;
    }
    // Bare IPs and localhost cannot have valid SSL certs — use plain http/ws
    secure = !(/^\d+\.\d+\.\d+\.\d+(:\d+)?$/.test(domain)) && domain !== "localhost";
  } catch (e) {}

  var http = secure ? "https" : "http";
  var ws   = secure ? "wss"   : "ws";
